...
import com.tribetails.auntieos.web.theme.DefaultAuntieDimensions
import com.tribetails.auntieos.web.theme.DefaultAuntieTypography
...
import com.tribetails.auntieos.web.theme.scaled
import com.tribetails.auntieos.web.theme.withAccent
...
/**
 * 17.1 theme-basics pure logic: accent / density / font-scale parse (fail-safe),
 * the theme-application math (withAccent / dims.scaled / typography.scaled), and
 * the UserProfile round-trip + rehydration helpers. The Compose Appearance panel
 * is thin over these.
 */
class ThemePersonalizationTest {

    // ---- parse: stored key -> enum, fail-safe to default ----
    ...
    @Test fun density_parse_failSafe() {
    ...
    @Test fun fontScale_parse_failSafe() {
    ...
    // ---- accent application (mode-correct) ----
    ...
    @Test fun withAccent_defaultOrange_matchesShippedPrimary() {
        // #9: the ORANGE default keeps the shipped brand primary unchanged (no drift).
        assertEquals(LightAuntieColors.primary, LightAuntieColors.withAccent(AccentChoice.DEFAULT).primary)
    }
    ...
    @Test fun withAccent_gradientChoice_setsBrush_andDrivesPrimaryFromGradientLead() {
        ...
        // A solid choice clears the brush.
    ...
    @Test fun density_scaled_changesSpacing_andNormalIsIdentity() {
        ...
        // Structural widths are intentionally NOT scaled.
        assertEquals(DefaultAuntieDimensions.sideRailWidth, roomy.sideRailWidth)
        assertEquals(DefaultAuntieDimensions.maxContentWidth, compact.maxContentWidth)
    }
    ...
    @Test fun fontScale_scaled_multipliesSizes_andMediumIsIdentity() {
        ...
        assertNotEquals(DefaultAuntieTypography.displayLarge.fontSize.value, large.displayLarge.fontSize.value)
    }

    // ---- UserProfile round-trip + rehydration ----
    ...
    @Test fun hydratedPersonalization_legacyDoc_allDefaults() {
        // A profile saved before 17.1 has blank fields -> every knob falls to default.
        val hydrated = hydratedPersonalization(UserProfile())
    ...
    @Test fun hydratedPersonalization_nullProfile_allDefaults() {
        val hydrated = hydratedPersonalization(null)
        assertEquals(AccentChoice.ORANGE, hydrated.accent)
        assertEquals(DensityChoice.NORMAL, hydrated.density)
        assertEquals(FontScaleChoice.MEDIUM, hydrated.fontScale)
    }
