 ...
 * screens/admin/MarkdownTemplate.kt. Subset: # / ## / ### headings, **bold**, _italic_,
 * [text](url), ![alt](url), "- " bullets, blank-line paragraphs, single newline -> <br>.
 * Handlebars {{vars}} pass through; HTML special chars escaped. See MarkdownTemplateTest.
 */
...
private fun delimAt(s: String, i: Int, delim: String, bold: Boolean): Span? {
    ...
    return Span(if (bold) MdInline.Bold(inner) else MdInline.Italic(inner), end + delim.length)
}
