...
//
// The old field was free text ("vip, monthly"), which was a quiet way to send a
// broadcast to nobody, because THREE case rules meet here and only one of them
// forgives a typo:
//
//   1. The tag VOCABULARY layer (data/model/TagModels.kt, ported from the React
...
//      no reason to expect tags to behave differently.
//
...
//
// SCOPE: household tags only. The server criteria schema has no pet-tag kind and
// the resolver reads `kinfolk.tags`, so offering pet tags here would build an
// audience the server ignores.
//
// This is the android twin of the commonMain BroadcastAudienceTags.kt. The two
// trees share no code, so a change to either needs the same change to the other;
// BroadcastAudienceTagsTest.kt pins the behavior on both sides.
// ─────────────────────────────────────────────────────────────────────────────
 ...
 *
 * The per-name length cap is the vocabulary's 40 (MAX_TAG_NAME_LENGTH), tighter
 * than the server's 60, so a name authored in the Tags editor can never trip the
 * server's own limit.
 */
 ...
 * casing seen wins, which is what the backend would have matched anyway).
 */
 ...
 * household doc carries and the server compares exactly. A blank name is a
 * no-op, a case-insensitive duplicate is a no-op, and the tag appends at the
 * end. Returns a new list; never mutates the input.
 *
...
fun unknownBroadcastTags(selected: List<String>, vocab: List<TagDef>): List<String> {
    ...
    return selected.filterNot { known.contains(nameKey(it)) }
}
 ...
 * would be wrong to hide is that the server compares tag text exactly, so a typo
 * here reaches nobody and reports a clean zero.
 *
