> STATUS 2026-08-09: the mock's remaining SUGGESTION, the **result-count chip**, is now SHIPPED on both platforms and is deliberately NOT gated, which supersedes the "Out of scope" bullet below (operator ruling today: build this screen "as mocked and all open items"). That bullet's reasoning was about the missing search BACKEND; a count over rows already on screen has no dead-input to ship, so it does not apply. Web: a mono pill on the `DenPanel("KinTales")` header reading `N loaded`, or `M of N loaded` once the tabs or the search box exclude something, and NOTHING at all while the first page is in flight or has failed. Android: the same two forms in `DenScreenHeading`'s trailing slot, plus a new `NoMatchState`: a search that excluded every row used to leave that screen blank with no explanation, indistinguishable from an empty workspace. The mock's other two SUGGESTIONs are settled: list search is live (below), and the "Refresh" affordance named in the mock's header comment is not DRAWN in the mock body and is not built (Android already pull-refreshes; web pages with "Load more"). Two stale paths in this spec: `web/composeApp/…` does not exist in this repo, the shipped web surface is the React admin at `auntieos-admin/src/screens/KinTales.tsx`; the mock's claim that the source has no list search has been false since the de-stale below.
