...
| Callables reaching **production** `us-central1` (nothing calls `connectFunctionsEmulator`), failing fast on one run and after a 20s `CallableTimeoutError` on the next | every non-localhost request is aborted, so a golden cannot depend on an external service being up. |
...
**What it does not photograph.** `payments` is skipped: `#/payments` names a slug the React nav table does not know (`lib/nav.ts`'s `routeWarning` says so), because Payments became a tab inside Settings. Screens whose data arrives through a callable photograph their **error state**, correctly and stably, because the functions emulator is not running: Templates and Form Schemas entirely, five of Home's seven widgets, the Inbox thread panel, ActivityLog's chain-integrity banner, KinTaleDetail's comment and reaction panels. Starting the functions emulator would change those screens, so do it deliberately and re-record, not by accident.
...
**`react` is the only surface with goldens today.** 19 ok, 0 regressions, recorded the day they were captured, after the non-determinism was pinned. The three Compose surfaces have captures and no goldens, so verifying them reports exit 2 until somebody records deliberately.
