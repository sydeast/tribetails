...
describe('swPolicy', () => {
  ...
  describe('navigation fallback', () => {
    ...
    it.each([
      ...
      expect(isDeniedNavigation(new URL(path, 'https://auntie.tribetails.com'))).toBe(false);
    });
    it('refuses the /api/* rewrites, which are functions wearing this hostname', () => {
      const base = 'https://auntie.tribetails.com';
      expect(isDeniedNavigation(new URL('/api/generate', base))).toBe(true);
      expect(isDeniedNavigation(new URL('/api/cloudinary/sign-upload', base))).toBe(true);
    });
