 ...
 * rows and the detail's linked-visits panel route to the visit's own route,
 * `/sessions/{id}` (#408, moved onto a path param by #753), and this screen is
 * unit-rendered without a router. Same stub as `Invites.test.tsx`.
 */
...
describe('Invoices row quick action', () => {
  ...
  it('fires the callable only once the confirm step is confirmed', async () => {
    sendInvoiceReminder.mockResolvedValue({ sent: true, lastReminderAtMs: 1, nextReminderAllowedAtMs: 2 });
    usePagedCollection.mockReturnValue(paged([entry({ status: 'open' })]));
    render(<Invoices />);
    await userEvent.click(within(rowOf('1042')).getByRole('button', { name: 'Send reminder' }));
    const dialog = screen.getByRole('dialog');
