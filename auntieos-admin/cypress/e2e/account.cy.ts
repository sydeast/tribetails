...
describe('account', () => {
  ...
  // Generated by Cypress Author, 2026-09-10. Selectors moved to the kit band
  // in the #755 sweep: the hero is `DenScreenHeading` now, the name is its h1
  // (`.den-heading-title`), the role line its `.den-heading-detail`, and the
  // badges are kit `.den-statuspill`s.
  it('#719 shows the signed-in login email and an Active badge in the hero', () => {
    cy.signIn();
    cy.visit('/account');
    cy.get('.account__fields', { timeout: BUDGET_MS.PROFILE_RENDER }).should('exist');

    cy.get('.account__hero .account__heroEmail').should('have.text', ADMIN.email);
    cy.contains('.account__hero .den-statuspill', 'Active').should('exist');
  });

  it('#755 draws the hero as the kit band with the mock crumb trail', () => {
    cy.signIn();
    cy.visit('/account');
    cy.get('.account__fields', { timeout: BUDGET_MS.PROFILE_RENDER }).should('exist');

    cy.get('.den-heading.account__hero').should('have.length', 1);
    cy.get('.account__hero .den-crumbs a').should('have.text', 'Settings');
    cy.get('.account__hero .den-crumbs [aria-current="page"]').should('have.text', 'User profile');
    cy.get('.account__hero .den-heading-leading').should('exist');
    cy.get('.account__hero button[aria-label="Change photo"]').should('exist');
  });
  ...
  it('saves an inline profile edit through Firestore and reads it back after a reload', () => {
    ...
    cy.get('.account__hero .den-heading-title').should('have.text', NEW_NAME);
    cy.get('.account__hero .den-heading-detail').should('contain.text', NEW_TITLE);
  ...
  it('#719 puts First name and Last name in the same row', () => {
    cy.signIn();
    cy.visit('/account');
    cy.get('.account__fields', { timeout: BUDGET_MS.PROFILE_RENDER }).should('exist');

    // The mock's side-by-side pair, not one full-width field above another:
    // both inputs share the same `.account__frow` row container.
    cy.get('#account-first-name')
      .closest('.account__frow')
      .within(() => {
        cy.get('#account-first-name').should('exist');
        cy.get('#account-last-name').should('exist');
      });
  });
  ...
  it('round-trips quotes, a tag, an emoji and newlines verbatim, and trims padding', () => {
    ...
    cy.get('.account__hero .den-heading-title').should('have.text', HOSTILE_NAME);
    cy.get('.account__hero .den-heading-title b').should('not.exist');
  });
  ...
  // Generated by Cypress Author, 2026-09-10.
  it('#719 gives the Business profile panel its own Save, separate from the hero button', () => {
    cy.signIn();
    cy.visit('/account');
    cy.get('.account__fields', { timeout: BUDGET_MS.PROFILE_RENDER }).should('exist');

    cy.contains('.den-panel', 'Business profile').within(() => {
      // Its own Save, disabled until this panel's own fields are dirty: a
      // shared button with the hero's "Save profile" would need no separate
      // control here at all.
  ...
  // Generated by Cypress Author, 2026-09-10.
  it('#719 shows all three notification channels as switches', () => {
    stubNotificationCallables();
    cy.signIn();
    cy.visit('/account');
    cy.get('[role="switch"][aria-label="Email notifications"]', {
      timeout: BUDGET_MS.PROFILE_RENDER,
    }).should('exist');
    cy.get('[role="switch"][aria-label="SMS notifications"]').should('exist');
    cy.get('[role="switch"][aria-label="Push notifications"]').should('exist');
  });
  ...
  describe('security', () => {
    ...
    // Generated by Cypress Author, 2026-09-10.
    // Presence only, neither button is clicked: "offers a reset link" above
    // already proves Send reset email works, and clicking Sign out here would
    // end the session before the LAST test below gets to be the one that does.
    it('#719 shows a Send reset email row and a Sign out row', () => {
      cy.signIn();
      cy.visit('/account');
      cy.contains('.security__rowTitle', 'Change password').should('exist');
      cy.contains('.security__row button', 'Send reset email').should('exist');
      cy.contains('.security__rowTitle', 'Sign out').should('exist');
      cy.contains('.security__row button', 'Sign out').should('exist');
    });
