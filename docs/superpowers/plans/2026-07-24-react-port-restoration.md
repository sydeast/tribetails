...
- Design references, in priority order: `auntieos-admin/page-specs/*.md`, `auntieos-admin/ui-ideas/*.html` (2026-05-27 concepts, minus everything under `WrongUIDesigns-UpdateKill/`), then the archive Compose source. **`auntieos-admin/visual/mockups/*` was third on this list and was deleted 2026-08-01:** it rendered the concept round before the operator sorted it, so it showed rejected designs with nothing marking them rejected. See `auntieos-admin/CLAUDE.md`.
