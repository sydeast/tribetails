...
| `backfill:tribal-intel-ids` | #460, #393 | historical, do not run | Legacy Tribal Intel rows pointed at a kinfolk by NAME (`kinfolkRef: "Jane Halbrook"`) instead of by id, so the nightly reconciliation pipeline's lookup missed them. `training_documents` is written only by that internal pipeline, not by the customer-data reload, and a full wipe clears both the stale rows and the kinfolk they would have matched against. See the flag below. |
...
**`backfill:tribal-intel-ids` can create duplicate people if it is run against
freshly re-uploaded data. Read this before ever running it again.** Its own
header states it has never been run against production. When it finds a name
matching no kinfolk, its answer is to CREATE a new `kinfolk` record for that
name - and creating one fires `onKinfolkCreate`, which provisions a full
`families/{id}` envelope behind it. A wipe and re-upload is exactly the
situation where a name that should match drifts instead: retyped spelling,
a middle name added or dropped, different casing between the old system and
the new one. Every one of those reads as "no match" and mints a phantom
person and a phantom household with nothing to undo it automatically. If this
script is ever run again, read every line under "KINFOLK RECORDS TO CREATE"
against the actual current roster by hand before adding `--allow-prod` -
never trust the dry run's own CREATE list as the last word here.
