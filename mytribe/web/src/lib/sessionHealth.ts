 ...
 * requests, the console capture holds no auth line at all, because nothing was
 * listening. `onAuthStateChanged` does not fire when a refresh fails — the SDK
 * keeps `currentUser` and keeps serving the cached token until it expires. The
 * SDK's own ProactiveRefresh (see @firebase/auth proactive_refresh.js) retries
 * a network failure on a 30s-to-16min backoff and silently gives up altogether
 * on any other error, telling nobody either way. So a refresh outage that
 * outlives the cached token leaves someone signed in, tapping, and being
 * refused — reading as "the app is broken" with no clue why.
 *
 * THIS FILE IS THE PORTAL'S COPY, and deliberately a copy rather than a shared
 * package: `lib/auth.ts` here and in the admin are already twins by hand for
 * the same reason (see its header). The two apps share no runtime code, have
 * different Firebase apps and different CSPs, and the drift risk of one more
 * parallel module is smaller than the coupling of a new shared workspace. The
 * kinfolk-facing copy is the portal's own — see components/SessionNotice.tsx.
 *
 ...
 * correct: while the cached token is still valid nothing anyone does is being
 * refused, and a notice then would be crying wolf. The session is only
 * called degraded once a refresh is genuinely needed and cannot be had.
 *
 * WHY A NOTICE RATHER THAN console + Sentry. PortalNav argues the opposite way
 * for the header logo, and is right to: a missing decoration has no degraded
 * state to disclose. This is a different kind of fact. A session that cannot
 * refresh will start refusing bookings and messages, and the kinfolk is the
 * only one who can fix it (sign in again) or wait it out knowingly. That
 * belongs on screen. It still reports to Sentry as well.
 */
...
/** Reactive session health for components (the portal's session notice). */
