...
/**
 * #454, the portal's half. The admin walk showed six consecutive securetoken
 * refresh failures inside one 13-second window, then a success — and not one
 * line anywhere saying so. The portal runs the same Firebase Auth SDK against
 * the same endpoint and had the same blind spot. These tests are the observer
 * that was missing: they drive a refresh that rejects and assert the session
 * says so, retries, and clears itself when the network comes back.
 */
...
describe('the session-health monitor', () => {
  ...
  it('clears itself when the network comes back, like the walk did', async () => {
    // The admin walk's shape exactly: six refusals, then a 200.
    const getIdToken = vi
      .fn()
