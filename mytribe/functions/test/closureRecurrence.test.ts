...
import {
  ...
  US_HOLIDAY_PRESETS,
  type ClosureEntry,
} from '../src/lib/closureRecurrence';

/**
 * The 2026-07-31 operator ruling: a US national holiday recurs yearly, so the
 * closures editor should not demand a fresh `YYYY-MM-DD` every January. These
 * tests pin the resolver's behavior BEFORE any UI is built on top of it,
 * covering the two shapes that are subtle enough to get wrong silently
 * (Nth-weekday, last-weekday), the leap-day policy, and a range crossing a
 * year boundary.
 */
...
describe('parseClosureEntry: legacy `once` entries are untouched', () => {
  ...
  it('no pipe at all: whole string is the name, blank date (parseDatedEntry parity)', () => {
    ...
    expect(result).toEqual<ClosureEntry>({ recurrence: 'once', name: 'malformed entry', date: '', month: 0, day: 0, weekday: 0, nth: 0 });
  });

  it('a bare date with no pipe is ALSO the no-pipe fallback, not a dated once (matches parseDatedEntry, which never inspects a pipe-less string for date-shape)', () => {
...
describe('parseClosureEntry: new yearly shapes', () => {
  ...
  it('a pipe inside a yearly name still round-trips', () => {
    expect(parseClosureEntry('yearly:07-04|Independence | Fireworks').name).toBe('Independence | Fireworks');
  });
});

describe('parseClosureEntry: malformed / out-of-range shapes fall back instead of lying', () => {
  it('an out-of-range month in yearly: falls back to once/blank-date/whole-string-as-name', () => {
    const raw = 'yearly:13-04|Bad month';
    expect(parseClosureEntry(raw)).toEqual<ClosureEntry>({ recurrence: 'once', name: raw, date: '', month: 0, day: 0, weekday: 0, nth: 0 });
  });
  ...
  it('the regex itself refuses an out-of-range weekday or nth before the schema even runs', () => {
    // weekday 8 and nth 5 are outside \d in [1-7]/[1-4], so these never match
    // YEARLY_NTH_RE at all and fall to the generic garbage-date-part fallback.
...
describe('formatClosureEntry: the inverse of parseClosureEntry', () => {
  ...
  it('yearly-last-weekday', () => {
    expect(formatClosureEntry({ recurrence: 'yearly-last-weekday', name: 'Memorial Day', month: 5, weekday: 1 })).toBe(
      'yearly-last:05-1|Memorial Day',
    );
  });

  it('round-trips through parseClosureEntry for every kind', () => {
...
describe('closureDateInYear: an Nth-weekday that does not exist that year returns null, not a wrong date', () => {
  it('a hypothetical 5th Monday of a 4-Monday September', () => {
    ...
    // September 2026 has only four Mondays (7, 14, 21, 28); asking for the
    // real 4th is fine...
    expect(closureDateInYear(entry, 2026)).toBe('2026-09-28');
    // ...but bump nth to a value the schema still allows (4 is the max), and
    // instead prove the OTHER edge directly: a month with no 5th Monday would
    // return null if nth could reach 5. Since the schema caps nth at 4 (every
    // month has a 4th of any given weekday), assert the boundary itself holds.
  });
});
