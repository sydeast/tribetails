...
- `saveTribeProfile`
  - req `{ kinfolkId?: string, displayName?: string /* 1..120 */, customFields?: CustomField[] /* max 1,985 */, removeCustomFieldKeys?: string[] /* 1..80 each, max 1,985 */ }`
...
- `saveHomeAccess`
  - req `{ kinfolkId?: string, gateCode?: string | null, keyLocation?: string | null, wifiPassword?: string | null, customFields?: CustomField[] /* max 1,985 */, removeCustomFieldKeys?: string[] /* 1..80 each, max 1,985 */ }`
  - res `{ ok: true }`
...
- LIMITS (#873 review). The old `max 40` rows and `label 1..80` locked households out: current clients send every stored row back, `getMyTribeProfile` serves a missing label as `''`, and `saveFormSchema` allows 200-character labels and 50 sections of 200 fields. Now (`src/lib/customFieldsMerge.ts`):
  ...
  - the request row cap, 1,985 (`CUSTOM_FIELDS_MAX_ROWS`), is the growth ceiling divided by the smallest row's JSON (33 bytes). No list the server writes can hold more rows, so any list it wrote can be sent back. It is below the 10,009 rows the schemas could name (50 x 200 plus 9 reserved rows); that many rows would outweigh the ceiling anyway, so bytes are the binding limit. A list written before this round with more than 1,985 rows could not be echoed by a current client; `report:truncated-custom-fields` counts the lists over 64 KiB, which is where any such list would be;
...
- RATE LIMIT (second review round). 60 saves an hour per household, through `enforceRateLimit` (`rate_limits/{scope}:{kinfolkId}`), refused with `resource-exhausted` and "Too many attempts. Try again later." Each callable has its OWN bucket, `profileSave` and `homeAccessSave`: one page Save calls both, so a shared bucket would allow 30 clicks an hour, and a click landing on its edge would save the profile and refuse the home details. It is counted after the access gate and outside the save transaction (which can run twice); a `saveTribeProfile` call with nothing to write spends no save. Web (`profileSaveErrorMessage`) and Android (`profileSaveFailureMessage`) show "Save failed: this household has saved too many times in the last hour. Wait a little, then save again."
