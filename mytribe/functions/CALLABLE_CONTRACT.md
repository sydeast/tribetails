...
- `convertTemplateToVisual` (#953, net-new, admin-gated like `saveTemplate`,
  ...
  - The web editor's Convert button. Reads the stored `emailTemplates/{id}`
    ...
    Save is the only write, same split `previewEmailTemplate` draws between
...
- AUDIT (#901). A save that WRITES records one `activity_log` entry after the transaction commits: `saveTribeProfile` a `PROFILE_UPDATED`, `saveHomeAccess` a `HOME_ACCESS_UPDATED` (its own event, because the two write different documents and `reportTruncatedCustomFields.ts` reads `PROFILE_UPDATED` as evidence that `families/{id}` itself was saved). Both carry FIELD NAMES ONLY, never a value, in `payload.fields`, with `customFields` standing for the whole list. `saveHomeAccess` names only the fields that actually CHANGED against what was stored, because both clients send all three scalars on every save and an entry reading "gateCode, keyLocation, wifiPassword" every time answers nothing. A gate code, key location, Wi-Fi password or custom row value must never reach `activity_log`, which has a wider reader set than the document it came from and is append-only once chained. A refused or no-op save writes no entry, and a failed audit write is logged (`audit.write.failed`) without failing a save that already committed.
...
- Already-lost rows: `scripts/reportTruncatedCustomFields.ts` (read-only). No before-state is recorded (PROFILE_UPDATED audit carries field names only, and `saveHomeAccess` wrote no audit at all until #901, which also carries field names only), so it lists lists that look like the old schema rebuild, with the portal-save evidence for each.
...
- req `{ kinfolk: Record<string, unknown>, ignoreDuplicateOf?: string }` (strict envelope). The household fields as each client already wrote them: web's six (`firstName`, `lastName`, `phoneNumber`, `email`, `status`, `serviceAddress`, plus `profilePictureUrl`/`joinDate` blanks), the Android and desktop models whole.
