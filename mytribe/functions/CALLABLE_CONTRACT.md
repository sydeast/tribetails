...
- Clients treat `duplicateOf` as "this household already exists": they continue Add Kinfolk against that id (save its Emergency Contact) and log no second CREATE audit (admin web writes none; desktop and Android skip theirs on a `duplicateOf` answer).
