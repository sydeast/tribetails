...
- Clients treat `duplicateOf` as "this household already exists": they continue Add Kinfolk against that id (save its Emergency Contact). Admin web writes no CREATE audit; the desktop console skips its CREATE audit on a `duplicateOf` answer. Admin Android does NOT yet: its CREATE audit lives in `DirectoryViewModel.saveKinfolk`, and `AuntieRepository.createKinfolkComplete` returns the existing id without saying it was a duplicate, so a `duplicateOf` there still logs "Created kinfolk" a second time.
