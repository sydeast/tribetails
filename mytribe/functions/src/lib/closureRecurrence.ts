 ...
 * read by three platforms (`auntieos-admin/src`, the Android app, and the now
 ...
 * decide whether a day is bookable -- not `bookingAvailability.ts` /
 * `api/availability.ts` (those check `businessHours`, `booking_time_slots`,
 * and `kin_care_sessions` only), not `Schedule.tsx`, not any MyTribe callable.
 * The closures list is operator-facing data with no functional consumer yet.
 * This resolver is written and tested against that future regardless: the
 * TimeOffEditor list display is ITS OWN consumer (it must show a sensible date
 * for a rule that has no single stored date), and a `date`/`range` consumer
 * added later has a shared, tested expansion to call rather than a fourth
 * reimplementation of "what is the 4th Thursday of November."
 */
 ...
 * `mergeBusinessSettings` convention in `auntieos-admin/src/api/settings.ts` --
 * a parser's output makes guarantees a raw cast does not.
 */
 ...
 * (`auntieos-admin/src/lib/settingsFormat.ts`) already gives a malformed
 * entry, so a corrupt or legacy-oddball row still renders as SOMETHING
 * instead of vanishing or crashing the settings read.
 *
 ...
 * property of the calendar date alone, not of where the process happens to be
 * running.
 */
 ...
 *
 * Ids are independent of `US_HOLIDAYS` in `auntieos-admin/src/lib/settingsFormat.ts`
 * (the older observed/not-observed checklist, a boolean per holiday with no
 * date anywhere in its storage). That list and this one serve different
 * fields -- `observedUsHolidays` vs. a dated `companyHolidays` entry -- so
 * sharing ids across them is not required and this module does not import
 * that one.
 */
