 ...
 * not run. `notifications` is wiped and regenerated by the app, not by the
 * re-upload, and `broadcastMessage.ts` already writes `category: def.category`
 * on every send - nothing written after a wipe can carry the dead category.
 */
import {
  getApps,
  initializeApp,
