 ...
 * not run. `notifications` is generated by the app's own crons and triggers,
 * not by the re-upload, and `dispatcher.ts` already writes every new
 * notification's work order straight to `notificationDispatch` - nothing
 * written after a wipe can carry the legacy fields this relocates.
 */
