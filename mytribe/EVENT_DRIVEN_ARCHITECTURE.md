...
There is NO AI generator inside MyTribe Cloud Functions today. AuntieOS copy is
generated by the n8n webhook `auntie-generate`. There is a migration plan to add a
MyTribe Cloud Function `generate` that is byte-compatible with the existing
`GenerateRequest` / `GenerateResponse` contract, feature-flagged, and blocked only on
the Anthropic API key (an external secret). Operator decision: `MyTribe/functions/`
is the canonical home for new callables. Until the key is provisioned, generation
stays on `auntie-generate`.
